...
<button class="btn btn-primary" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasWithBothOptions" aria-controls="offcanvasWithBothOptions" style="margin-left:-120px; width:150px; height:80px; text-align:center; background-color:#E88EBA">
...
function BuscarUsuario2() {
  var buscador=$('#buscarusuario').val();
  var usu=$('#usu').val();
jQuery.ajax({
url: 'ajax-usubusca2.php',
data: 'buscarusuario=' + buscador +'&usu='+ usu,
type: "POST",
success:function(data){
  $("#buscausuario").html(data);
},
error:function (){}
});
}
...
<!-- NAVEGADOR -->
<nav class="navbar navbar-expand-lg navbar-dark bg-dark" style="margin-top:-90px; margin-left:-50px; width:1350px;">
            <ol >
            ...
            <form method="post" class="form-inline" style="margin-left: 300px; margin-top:10px;">
    
    <input class="form-control mr-sm-2" list="modelslist" type="search" name="buscarusuario" id="buscarusuario" placeholder="Usuario,nombre,apellido..." aria-label="Search" style="margin-left:50px; width: 450px; font-size:20px; height:40px;" onkeydown="BuscarUsuario2()" onkeyup="BuscarUsuario()">
    <input type="hidden" name="usu" id="usu" value="<?php echo $data["ID_usuario"]?>">
...
echo "<button type='button' class='btn btn-primary' data-toggle='modal' data-target='#exampleModalCenter' style='margin-top:-50px; margin-left:580px; background-color:#D07118'>
...
<div style="margin-top:-40px; margin-left:1100px;" id="navperfil">
            <h6 class="tucuenta" style="color:white; margin-left:-50px;">Usuario: <?php echo $data["ID_usuario"] ?></h6>
            <h4 class="tucuenta" style="color:white; margin-left:-50px;">Hola,<?php echo $data["nombre"] ?></h4>
  ...
  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="margin-left:1200px; font-size:15px;">
    <a class="dropdown-item" href="EditarPerfil.php?usu=<?php echo $data['ID_usuario'] ?>" >Editar Perfil</a>
